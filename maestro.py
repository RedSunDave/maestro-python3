...
class Controller():
	...
	def set_pwm(self, target_ontime, target_period):
		"""
		Set up the Pulse Width Modulation on the 12th Pin
		"""

		lsb_ontime = target_ontime & 0x7f #7 bits for least significant byte
		msb_ontime = (target_ontime >> 7) & 0x7f #shift 7 and take next 7 bits for msb
		lsb_period = target_period & 0x7f #7 bits for least significant byte
		msb_period = (target_period >> 7) & 0x7f #shift 7 and take next 7 bits for msb
		cmd = chr(0x0A) + chr(lsb_ontime) + chr(msb_ontime) + chr(lsb_period) + chr(msb_period)
		self.send_command(cmd)
		# Record Target value
		#self.targets[chan] = target
